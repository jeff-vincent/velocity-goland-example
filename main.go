...
func getPost(ctx *gin.Context, mongoClient *mongo.Client, title string) (bson.D, error) {
	...
	if err != nil {
		log.Error().Err(err).Msg("error occurred while fetching posts from posts mongo")
...
func Publish(ctx *gin.Context, payload string) {
	...
	if err != nil {
		log.Error().Err(err).Msg("error occurred while connecting to redis")
	...
	if err := rdb.RPush(ctx, "queue:blog-view", payload).Err(); err != nil {
		log.Error().Err(err).Msg("error occurred while publishing to redis")
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "Analytics error"})
		return
	}
}
...
func main() {
	...
	if err != nil {
		log.Error().Err(err).Msg("error occurred while connecting to mongo")
	}
	...
	router.GET("/posts/:title", func(ctx *gin.Context) {
		...
		if err != nil {
			log.Error().Err(err).Msg("error occurred while fetching post   ")
		}
		fmt.Print(`new line \n`)
